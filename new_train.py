...
cfgs = {
    ...
    'weight_decay': 0,
    'momentum': 0.9,
    # 'snapshot': 'iter_30000_loss_0.01355_lr_0.000144',
    'snapshot': '',
    'val_freq': 5000,
    'crop_size': 256
}
...
def train(net, optimizer, writer):
    ...
    # tr = trange(cfgs['last_iter'], cfgs['iter_num'], desc="Training", dynamic_ncols=True)
    ...
    while curr_iter <= cfgs['iter_num']:
        ...
        print('%d / %d' % (curr_iter, cfgs['iter_num']))

        for data in tqdm(train_loader, desc="Training", dynamic_ncols=True):
            ...
            loss = loss_x_jf + loss_x_j0 + loss_x_j1 + loss_x_j2 + loss_x_j3 + loss_x_j4 \
                   + 10 * loss_t + loss_a
            # loss = loss_x_jf + 10 * loss_t + loss_a
            loss.backward()
            ...
            # log = '[iter %d], [train loss %.5f], [loss_x_fusion %.5f], [loss_t %.5f], [loss_a %.5f], ' \
            #       '[lr %.13f]' % \
            #       (curr_iter, train_loss_record.avg, loss_x_jf_record.avg, loss_t_record.avg, loss_a_record.avg,
            #        optimizer.param_groups[1]['lr'])
            # print(log)
            writer.add_scalar('train/loss', train_loss_record.avg, curr_iter)
            writer.add_scalar('train/loss_x_fusion', loss_x_jf_record.avg, curr_iter)
            writer.add_scalar('train/loss_x_phy', loss_x_j0_record.avg, curr_iter)
...
def validate(net, curr_iter, optimizer, writer):
    ...
    writer.add_scalar('val/loss', loss_record.avg, curr_iter + 1)
