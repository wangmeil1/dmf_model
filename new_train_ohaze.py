...
from new_model import new_model_OHAZE
...
def main():
    logdir = 'runs/model-' + datetime.datetime.now().strftime('%y%m%d-%H%M%S')
    writer = SummaryWriter(log_dir=logdir)
    shutil.copy('new_model.py', os.path.join(logdir, 'new_model.py'))
    net = new_model_OHAZE().cuda().train()
...
def train(net, optimizer, writer):
    ...
    loop = tqdm(range(curr_iter + 1, cfgs['iter_num'] + 1))

    for (_, data) in zip(loop, cycle(train_loader)):
        ...
        with torch.cuda.amp.autocast(enabled=True) as autocast, torch.backends.cuda.sdp_kernel(
        ...
        log = '[iter %d], [train loss %.5f], [loss_x_fusion %.5f], [loss_x_j1 %.5f], ' \
              ...
              (curr_iter, loss, loss_x_jf, loss_x_j1, loss_x_j2, loss_x_j3, loss_x_j4,
               optimizer.param_groups[1]['lr'])
        # print(log)
        writer.add_scalar('train/loss', loss.item(), curr_iter)
        writer.add_scalar('train/loss_x_jf', loss_x_jf.item(), curr_iter)
        writer.add_scalar('train/loss_x_j1', loss_x_j1.item(), curr_iter)
        writer.add_scalar('train/loss_x_j2', loss_x_j2.item(), curr_iter)
        writer.add_scalar('train/loss_x_j3', loss_x_j3.item(), curr_iter)
        writer.add_scalar('train/loss_x_j4', loss_x_j4.item(), curr_iter)
        writer.add_scalar('train/lr', optimizer.param_groups[1]['lr'], curr_iter)
        open(log_path, 'a').write(log + '\n')

        if (curr_iter + 1) % cfgs['val_freq'] == 0:
            validate(net, curr_iter, optimizer, writer)
            torch.cuda.empty_cache()
...
def validate(net, curr_iter, optimizer, writer):
    print('validating...')
    net.eval()
    ...
    # print(log)
    writer.add_scalar('validate/loss', loss_record.avg, curr_iter+1)
    writer.add_scalar('validate/psnr', psnr_record.avg, curr_iter+1)
    writer.add_scalar('validate/ssim', ssim_record.avg, curr_iter+1)
    open(log_path, 'a').write(log + '\n')
